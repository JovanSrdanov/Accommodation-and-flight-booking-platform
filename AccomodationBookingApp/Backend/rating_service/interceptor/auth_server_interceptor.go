...
func (interceptor *AuthServerInterceptor) authorize(ctx context.Context, method string) (error, context.Context) {
	...
	if providedRole == model.ROLE_UNKNOWN {
		log.Println("access attempt from an unknown service")
	...
	log.Println("unauthorized access attempt, the service does not have a valid role")
	return status.Error(codes.PermissionDenied, "no permission to access this RPC"), nil
}
...
func getProtectedMethodsWithAllowedRoles() map[string][]model.ServiceRole {
	...
	return map[string][]model.ServiceRole{
		authServicePath + "RateAccommodation":            {model.ROLE_API_GATEWAY},
		authServicePath + "RateHost":                     {model.ROLE_API_GATEWAY},
		authServicePath + "DeleteRatingForAccommodation": {model.ROLE_API_GATEWAY},
		authServicePath + "DeleteRatingForHost":          {model.ROLE_API_GATEWAY},
	}
}
