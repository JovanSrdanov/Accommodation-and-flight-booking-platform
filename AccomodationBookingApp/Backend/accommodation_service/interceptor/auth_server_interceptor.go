...
func getProtectedMethodsWithAllowedRoles() map[string][]model.ServiceRole {
	...
	return map[string][]model.ServiceRole{
		...
		authServicePath + "Create":         {model.ROLE_API_GATEWAY},
		authServicePath + "DeleteByHostId": {model.ROLE_API_GATEWAY},
		authServicePath + "GetById":        {model.ROLE_API_GATEWAY, model.ROLE_RATING_SERVICE},
	}
}
